...
export function CollectionDashboard({ config, configs }: Props) {
  ...
  return <div className="min-h-dvh bg-[#f5f7fa] text-[#17344d]">
    <header className="flex min-h-[4.25rem] items-center gap-4 border-b border-[#dce5ec] bg-white px-5 shadow-[0_2px_10px_rgba(15,65,101,0.07)]">
      ...
      <div className="ml-auto flex items-center gap-3"><a href="/monitoring" className="flex h-9 items-center gap-2 rounded-md border border-[#d5e0e8] px-3 text-xs font-semibold text-[#526d82] hover:bg-[#f6fafc]"><span className="material-symbols-outlined text-[17px]">sensors</span>Trạm quan trắc</a><a href="/overview" className="flex h-9 items-center gap-2 rounded-md border border-[#d5e0e8] px-3 text-xs font-semibold text-[#526d82] hover:bg-[#f6fafc]"><span className="material-symbols-outlined text-[17px]">analytics</span>Tổng quan thống kê</a><span className="hidden h-9 w-9 items-center justify-center rounded-full bg-[#e6f1fb] text-[#1467ad] sm:flex"><span className="material-symbols-outlined">person</span></span></div>
    </header>
    <div className="relative flex min-h-[calc(100dvh-4.25rem)]">
      ...
      <main className="min-w-0 flex-1 px-4 py-5 sm:px-6 lg:px-7">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em]" style={{ color: activeConfig.color }}>{activeConfig.kicker}</p><h1 className="mt-1 text-lg font-extrabold text-[#15324d]">{activeConfig.title}</h1><p className="mt-1 text-xs text-[#718596]">{activeConfig.description}</p></div><button type="button" onClick={() => void handleExport()} disabled={exporting} className="flex h-9 items-center gap-2 rounded-md border border-[#1474b8] px-3 text-xs font-bold text-[#12649d] hover:bg-[#edf7fd] disabled:opacity-50"><span className="material-symbols-outlined text-[17px]">description</span>{exporting ? "Đang xuất..." : "Xuất báo cáo"}</button></div>
        <div className="mb-3 flex flex-wrap items-center gap-2"><label className="flex h-9 min-w-[15rem] flex-1 items-center gap-2 rounded-md border border-[#d5e0e8] bg-white px-3 focus-within:border-[#1681c7] sm:max-w-[25rem]"><span className="material-symbols-outlined text-[18px] text-[#7892a4]">search</span><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder={activeConfig.searchPlaceholder} className="min-w-0 flex-1 text-xs outline-none placeholder:text-[#95a6b2]" /></label><select value={collection} onChange={(event) => reset(event.target.value)} className="h-9 min-w-[13rem] rounded-md border border-[#d5e0e8] bg-white px-3 text-xs text-[#526d82] outline-none">{activeConfig.collections.map((item) => <option key={item.collection} value={item.collection}>{item.label}</option>)}</select>{selectedConfig.filterField && <input value={filterInput} onChange={(event) => setFilterInput(event.target.value)} placeholder={selectedConfig.filterLabel} className="h-9 min-w-[10rem] rounded-md border border-[#d5e0e8] bg-white px-3 text-xs text-[#526d82] outline-none placeholder:text-[#95a6b2]" />}<button type="button" onClick={() => reset()} className="h-9 rounded-md px-2 text-xs font-semibold text-[#687f90] hover:bg-white">Xóa lọc</button></div>
        <section className="overflow-hidden rounded-lg border border-[#dce5eb] bg-white shadow-[0_3px_14px_rgba(23,60,88,0.05)]"><div className="flex items-center justify-between border-b border-[#e5edf2] px-3 py-2.5 text-xs text-[#718596]"><span>Hiển thị {result.items.length} trong tổng số {result.total === null ? "-" : result.total.toLocaleString()} kết quả</span><span>Trang {page}/{totalPages}</span></div>{error && <div className="m-3 rounded-md border border-[#f0c7c7] bg-[#fff5f5] px-3 py-2 text-xs text-[#b42318]">{error}</div>}<div className="overflow-x-auto"><table className="w-full min-w-[850px] border-collapse text-left text-xs"><thead className="bg-[#fbfcfd] text-[10px] font-bold uppercase tracking-wide text-[#62788a]"><tr>{selectedConfig.columns.map((column) => <th key={column.field} className="px-3 py-3">{column.label}</th>)}</tr></thead><tbody className="divide-y divide-[#edf1f4]">{loading ? <tr><td colSpan={selectedConfig.columns.length} className="px-3 py-12 text-center text-[#718596]">Đang tải dữ liệu...</td></tr> : result.items.length === 0 ? <tr><td colSpan={selectedConfig.columns.length} className="px-3 py-12 text-center text-[#718596]">Không tìm thấy dữ liệu phù hợp.</td></tr> : result.items.map((item) => <tr key={String(recordId(item))} onClick={() => setSelected(item)} className={`cursor-pointer hover:bg-[#f4f9fc] ${selected && recordId(selected) === recordId(item) ? "bg-[#eaf5fc] shadow-[inset_2px_0_0_#1681c7]" : ""}`}>{selectedConfig.columns.map((column) => { const value = column.render ? column.render(item) : display(item[column.field]); return <td key={column.field} className="max-w-[15rem] truncate px-3 py-3" title={value}>{value}</td>; })}</tr>)}</tbody></table></div><div className="flex items-center justify-between border-t border-[#e5edf2] px-3 py-2.5"><span className="text-[11px] text-[#718596]">{selectedConfig.label}</span><div className="flex gap-1"><PageButton label="chevron_left" disabled={page <= 1 || loading} onClick={() => setPage((current) => current - 1)} /><span className="flex h-7 min-w-7 items-center justify-center rounded bg-[#0878bd] px-2 text-[11px] font-bold text-white">{page}</span><PageButton label="chevron_right" disabled={page >= totalPages || loading} onClick={() => setPage((current) => current + 1)} /></div></div></section>
      </main>
...
function DetailPanel({ item, config, color, onClose }: { item: CatalogRecord; config: CatalogCollectionConfig; color: string; onClose: () => void }) {
  ...
  return <aside className="fixed inset-y-[4.25rem] right-0 z-20 w-full max-w-[25rem] overflow-y-auto border-l border-[#dce5eb] bg-white shadow-[-6px_0_20px_rgba(23,60,88,0.1)] sm:absolute sm:inset-y-[4.25rem]">
    <div className="flex items-start justify-between border-b border-[#e5edf2] px-5 py-4"><div><p className="text-[10px] font-bold uppercase tracking-wide text-[#1474b8]">Chi tiết dữ liệu</p><h2 className="mt-1 text-base font-extrabold text-[#15324d]">{display(item[config.titleField])}</h2></div><button type="button" onClick={onClose} title="Đóng" className="text-[#718596]"><span className="material-symbols-outlined">close</span></button></div>
    {geometry && <div className="border-b border-[#e5edf2] px-5 py-4">
      <FeatureMiniMap geometry={geometry} color={color} />
      <a href={`/?focus=${encodeURIComponent(config.collection)}:${encodeURIComponent(String(id))}`} className="mt-3 flex h-9 items-center justify-center gap-2 rounded-md bg-[#0878bd] text-xs font-bold text-white hover:bg-[#075f9e]">
