...
/** Reads `field_en` or `translations.en.field`, then falls back to Vietnamese. */
export function getLocalizedDataValue(properties: DataRecord, fieldNames: string[], language: SupportedLanguage): LocalizedDataValue {
  if (language !== "vi") {
    const translations = properties.translations;
    const languageRecord = translations && typeof translations === "object"
...
export function appLanguage(language: string): SupportedLanguage {
  const shortCode = language.split("-")[0] as SupportedLanguage;
  return ["vi", "en", "fr", "ja", "ko", "zh"].includes(shortCode) ? shortCode : "vi";
}
